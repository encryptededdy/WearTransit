...
class ATAPI {
    ...
    private val retrofitRT = Retrofit.Builder()
            .baseUrl(ENDPOINT)
            .addConverterFactory(GsonConverterFactory.create())
            .build()
    ...
    fun getRTBoard(stop: Stop, hours: Int, listener: RTBoardListener) {
        val call = retrofitRT.realtimeBoard(stop.stop_code, ATConstants.realtime_key, hours)
        call.enqueue(object : Callback<ATResponse<ATRTResponse>> {
            override fun onFailure(call: Call<ATResponse<ATRTResponse>>?, t: Throwable?) {
                Log.e("AT API", t?.message)
                listener.update(null)
            }

            override fun onResponse(call: Call<ATResponse<ATRTResponse>>?, response: Response<ATResponse<ATRTResponse>>?) {
                val body: ATResponse<ATRTResponse>? = response?.body()
                if (response != null && response?.isSuccessful && body != null && body.status == "OK") {
                    listener.update(body.response.movements)
                } else {
                    listener.update(null)
                    Log.e("AT API", response?.errorBody().toString())
                }
